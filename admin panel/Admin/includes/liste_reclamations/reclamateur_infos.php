...
if (isset($_POST['reclamateur_id'])) {

    $reclamateur_infos = $database->prepare(" SELECT * FROM user WHERE id_user=" . $_POST['reclamateur_id'] . "");
    $reclamateur_infos->execute();

    $response = '<table border="1" class="table table-bordered text-dark">';

    foreach ($reclamateur_infos as $resultat) {
